...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (_commands.size() > 0  && commands.size() > 0) {
            throw new IllegalStateException("Error: More than one command for "
                + getTarget());
        }
    ...
    private void checkFinishedDependencies() {
        for (int ruleNum : _depends.successors(_vertex)) {
            Rule succ = _depends.getLabel(ruleNum);
            if (succ.isUnfinished()) {
                error("Error : Circularity involving %s and %s",
                    _target, succ.getTarget());
            }
        }
    ...
    private boolean outOfDate() {
        ...
        for (int ruleNum : _depends.successors(_vertex)) {
            if (_depends.getLabel(ruleNum).getTime() > _time) {
                return true;
            }
        }
        return false;
